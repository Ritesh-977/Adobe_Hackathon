...
#     # Process each page with PyMuPDF (for text PDFs)
...
#                     if line_text.strip():
#                         # Use mean font size in this line
...
#     # Check if text-based extraction worked (else use OCR)
#     if not sorted_sizes:
#         # No extractable text: fallback to OCR
#         return extract_outline_ocr(pdf_path)
...
#         # If images can't be extracted (corrupt PDF, etc)
#         return {
#             "title": os.path.splitext(os.path.basename(pdf_path))[0],
#             "outline": []
#         }
...
#     # Heuristic: Find headings from OCR blocks
#     outline = []
#     title = None
...
#             # Skip lines that are very short or purely numeric or junk
#             if len(line) < 3 or line.isnumeric(): continue

#             # Title: First big line on page 1, mostly uppercase or Title Case, short
#             if (not title and page == 1 and idx <= 5 and 
#                 (line.isupper() or line.istitle()) and 3 <= len(line.split()) <= 12):
#                 title = line.strip()

#             # H1: ALL CAPS and not the title, short
#             if line.isupper() and len(line.split()) <= 10 and line != title:
#                 outline.append({"level": "H1", "text": line.strip(), "page": page})
#             # H2: Title Case, short to medium
#             elif line.istitle() and len(line.split()) <= 12 and line != title:
#                 outline.append({"level": "H2", "text": line.strip(), "page": page})
#             # H3: Section-like lines (starting with a number)
#             elif (len(line.split()) <= 14 and
#                   (line.strip()[0].isdigit() or line.istitle()) and 
#                   line != title):
#                 outline.append({"level": "H3", "text": line.strip(), "page": page})

#     if not title:
#         title = outline[0]["text"] if outline else os.path.splitext(os.path.basename(pdf_path))[0]

#     # If lots of duplicates, you may want to postprocess outline for uniqueness
#     seen_headings = set()
#     deduped_outline = []
#     for item in outline:
